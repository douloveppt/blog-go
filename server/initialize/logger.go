...
var (
	...
	Record = traceRecorder{Interface: Default, BeginAt: time.Now()}
)
...
func (c *customLogger) Printf(message string, data ...interface{}) {
	if global.GCONFIG.Mysql.LogZap != "" {
		switch len(data) {
		case 0:
			global.GLOG.Info(message)
		case 1:
			global.GLOG.Info("gorm", zap.Any("src", data[0]))
		case 2:
			global.GLOG.Info("gorm", zap.Any("src", data[0]), zap.Any("duration", data[1]))
		case 3:
			global.GLOG.Info("gorm", zap.Any("src", data[0]), zap.Any("duration", data[1]), zap.Any("rows", data[2]))
		case 4:
			global.GLOG.Info("gorm", zap.Any("src", data[0]), zap.Any("duration", data[1]), zap.Any("rows", data[2]), zap.Any("sql", data[3]))
		}
		return
	}
	switch len(data) {
